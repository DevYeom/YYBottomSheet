...
@available(iOS 9.0, *)
public class YYBottomSheet: UIViewController, UITableViewDelegate, UITableViewDataSource {
	public typealias SelectHandler = (YYBottomSheetCell) -> ()
		...
		didSet {
			...
			if let weakSelf = weakSelf {
				if allowTouchOutsideToDismiss == true {
					weakSelf.tapOutsideTouchGestureRecognizer.addTarget(weakSelf, action: #selector(YYBottomSheet.dismissView))
				} else {
					weakSelf.tapOutsideTouchGestureRecognizer.removeTarget(weakSelf, action: #selector(YYBottomSheet.dismissView))
				}
			}
		}
	}
	...
	public var headerViewTitleLabelTextColor: UIColor = YYBottomSheet.HeaderViewTitleLabelTextColor {
		didSet {
			weak var weakSelf = self
			if let weakSelf = weakSelf {
	...
	func setupViews() {
		...
		// Setup Gesture
		if allowTouchOutsideToDismiss == true {
			self.tapOutsideTouchGestureRecognizer.addTarget(self, action: #selector(YYBottomSheet.dismissView))
		}
		...
		self.headerView.backgroundColor = YYBottomSheet.HeaderViewBackgroundColor
		self.headerViewTitleLabel.text = self.headerViewTitle
		self.headerViewTitleLabel.font = UIFont.boldSystemFont(ofSize: 18)
		self.headerViewTitleLabel.textColor = YYBottomSheet.HeaderViewTitleLabelTextColor
		let closeButtonImage = UIImage(named: "close_button", in: Bundle(for: YYBottomSheet.self), compatibleWith: nil)
	...
	override public func viewDidAppear(_ animated: Bool) {
		...
		// bottom-up animation
		UIView.animate(withDuration: YYBottomSheet.ShowDuration, animations: { () -> Void in
			self.view.layoutIfNeeded()
		})
	}

	// MARK: - YYBottomSheet Usage
	public func show() {
		if viewNotReady() {
			return
		}
	...
	@objc func dismissView() {
		...
		// top-down animation
		UIView.animate(withDuration: YYBottomSheet.HideDuration, animations: {
			self.view.alpha = 0
			self.view.layoutIfNeeded()
	...
	public func selectCell(_ sender: YYBottomSheetCell) {
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell = tableView.dequeueReusableCell(withIdentifier: YYBottomSheetCell.identifierForTableViewCell, for: indexPath) as! YYBottomSheetCell
	...
	public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		let cell = tableView.cellForRow(at: indexPath) as! YYBottomSheetCell
